...
if(isset($_GET['no_surat'])){
    ...
    if(mysqli_num_rows($cek_data) > 0){
        ...
        // Update status siswa menjadi 'pindah' setelah surat pindah berhasil dibuat
        if($insert_surat_keluar) {
            $update_status_siswa = mysqli_query($conn, "UPDATE siswa SET status = 'pindah' WHERE nis = '$nis'");
        }
    }
...
<center class="no-print">
    ...
    <div style="display: flex; justify-content: center; align-items: center; gap: 10px;">
        ...
        if(isset($_GET['no_surat'])){
        ?>
            <form action="<?= BASE_URL ?>/pages/laporan/list_pindah.php" style="margin: 0;">
                <button type="submit">
